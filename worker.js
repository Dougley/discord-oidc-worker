...
app.post("/token", async (c) => {
  ...
  let scopesAuthorized = r["scope"].split(" ");
  ...
  console.log(
    scopesAuthorized,
    scopesAuthorized.includes("guilds.members.read"),
    "serversToCheckRolesFor" in config,
    config.serversToCheckRolesFor,
    servers.includes(config.serversToCheckRolesFor[0])
  );
  if (
    scopesAuthorized.includes("guilds.members.read") &&
    "serversToCheckRolesFor" in config
  ...
  const idToken = await new jose.SignJWT({
    ...
    nbf: Math.floor(new Date() / 1000) - 5,
    amr: userInfo.mfa_enabled ? ["pwd", "mfa"] : ["pwd"],
    preferred_username,
    ...userInfo,
    ...roleClaims,
